...
const Register = () => {
  ...
  const onSubmit = async (data) => {
    ...
    setLoading(true);
    try {
      const user = await authRegister(data.name, data.email, data.password, data.role);
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-black flex items-center justify-center p-4 transition-colors duration-300">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-1/3 -right-32 w-96 h-96 bg-purple-500/10 rounded-full blur-3xl"></div>
        <div className="absolute bottom-1/3 -left-32 w-96 h-96 bg-indigo-500/10 rounded-full blur-3xl"></div>
      </div>
      ...
      <div className="w-full max-w-md relative">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-gray-400 dark:text-slate-500 mt-1">Join WeeklyPulse</p>
        </div>
        ...
        <div className="bg-white/80 dark:bg-slate-800/40 backdrop-blur-xl border border-gray-200 dark:border-slate-700/50 rounded-2xl p-8 shadow-xl shadow-gray-200/50 dark:shadow-none transition-colors duration-300">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            {/* Name */}
            <div>
              <label className="block text-xs font-medium text-gray-500 dark:text-slate-400 mb-1.5">Full Name</label>
            ...
            {/* Role */}
            <div>
              <label className="block text-xs font-medium text-gray-500 dark:text-slate-400 mb-2">Role</label>
              <div className="grid grid-cols-2 gap-3">
                <label className="relative cursor-pointer">
                  ...
                  />
                  <div className="px-4 py-3 bg-gray-50 dark:bg-slate-900/50 border border-gray-200 dark:border-slate-700/50 rounded-xl text-center peer-checked:border-indigo-500/50 peer-checked:bg-indigo-50 dark:peer-checked:bg-indigo-500/10 transition-all">
                    <p className="text-sm font-medium text-gray-600 dark:text-slate-300 peer-checked:text-indigo-600 dark:peer-checked:text-indigo-400">
                      Team Member
                    </p>
                    <p className="text-[10px] text-gray-400 dark:text-slate-600 mt-0.5">Submit reports</p>
                  </div>
                </label>
                <label className="relative cursor-pointer">
                  <input
                    ...
                    className="peer sr-only"
                  />
                  <div className="px-4 py-3 bg-gray-50 dark:bg-slate-900/50 border border-gray-200 dark:border-slate-700/50 rounded-xl text-center peer-checked:border-purple-500/50 peer-checked:bg-purple-50 dark:peer-checked:bg-purple-500/10 transition-all">
                    <p className="text-sm font-medium text-gray-600 dark:text-slate-300 peer-checked:text-purple-600 dark:peer-checked:text-purple-400">
                      Manager
                    </p>
                    <p className="text-[10px] text-gray-400 dark:text-slate-600 mt-0.5">View & analyze</p>
                  </div>
                </label>
              </div>
            </div>
